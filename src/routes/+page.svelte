<script lang="ts">
	...
	function onFuelSelection(event: CustomEvent<selectedOption>) {
		console.log('selecting fuel: ', event.detail.value);
		$selectedFuels = [...$selectedFuels, event.detail.value];
	...
	function onFuelRemove(fuel: String) {
		console.log('remove fuel ', fuel);
		$selectedFuels = $selectedFuels.filter((e) => e !== fuel);
		$selectedFuels = $selectedFuels;
	}
...
<div class="container h-full items-center space-y-8 p-4">
	...
	<section class="space-y-1">
		...
		<select class="select w-full min-w-[120px]" multiple bind:value={$selectedOutputs}>
